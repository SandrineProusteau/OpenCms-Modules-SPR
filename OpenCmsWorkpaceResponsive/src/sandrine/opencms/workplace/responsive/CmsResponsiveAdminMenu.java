 ...
 * @since 6.0.0 
 */
public class CmsResponsiveAdminMenu extends CmsToolDialog {

    /** Default link target constant. */
    public static final String DEFAULT_TARGET = "admin_content";

    /** Group container. */
    private CmsIdentifiableObjectContainer<CmsResponsiveAdminMenuGroup> m_groupContainer = new CmsIdentifiableObjectContainer<CmsResponsiveAdminMenuGroup>(
        true,
        true);
     ...
     */
    public CmsResponsiveAdminMenu(CmsJspActionElement jsp) {
     ...
     */
    public void addGroup(CmsResponsiveAdminMenuGroup group) {
     ...
     */
    public void addGroup(CmsResponsiveAdminMenuGroup group, float position) {
     ...
     * @return the new item
     */
    public CmsResponsiveAdminMenuItem addItem(
        String groupName,
        String name,
        String icon,
        String link,
        ...
        CmsResponsiveAdminMenuGroup group = getGroup(groupName);
        if (group == null) {
            String gid = "group" + m_groupContainer.elementList().size();
            group = new CmsResponsiveAdminMenuGroup(gid, groupName);
            addGroup(group, position);
        }
        String id = "item" + group.getId() + group.getMenuItems().size();
        CmsResponsiveAdminMenuItem item = new CmsResponsiveAdminMenuItem(id, name, icon, link, helpText, enabled, target);
     ...
     */
    public CmsResponsiveAdminMenuGroup getGroup(String name) {
    ...
    public String groupHtml(CmsWorkplace wp) {
        ...
        Iterator<CmsResponsiveAdminMenuGroup> itHtml = m_groupContainer.elementList().iterator();
        while (itHtml.hasNext()) {
            CmsResponsiveAdminMenuGroup group = itHtml.next();
    ...
    public void installMenu() {
        ...
        CmsResponsiveAdminMenuGroup helpMenu = new CmsResponsiveAdminMenuGroup("help", Messages.get().getBundle(getLocale()).key(
            Messages.GUI_ADMIN_MENU_HELP_GROUP_0));
        helpMenu.addMenuItem(new CmsResponsiveAdminContextHelpMenuItem());
        addGroup(helpMenu);
        ...
        while (itElems.hasNext()) {
            ...
            if (path.indexOf(CmsToolManager.TOOLPATH_SEPARATOR, pos + 1) < 0) {
                ...
                addItem(
                    ...
                    CmsResponsiveAdminMenu.DEFAULT_TARGET);
            }
        }
    }
